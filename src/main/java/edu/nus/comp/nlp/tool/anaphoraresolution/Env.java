...
class Env {
  ...
  static {
    ...
    // the file stores the environment variables (path to the Charniak parser,
    // etc.)
    String environmentFileName = classLoader.getResource("env.jrap").getFile();
    ...
    try {
      ...
      if (!environmentFile.exists()) {
        // can't find the environment file, first try to find it under the
        // directory were the jar file is stored
      ...
      if (!environmentFile.exists()) {
        System.err
            .println("Can't find the file \""
                + environmentFile.getCanonicalPath()
                + "\", where environment variables are stored. It should be in the directory where JavaRAP is invoked.");
        System.exit(-1);
      }

      environmentFileName = environmentFile.getCanonicalPath();

      String[] records = Util.readFile(environmentFileName).toString()
      ...
      if (parserHomeDir == null
          ...
          || tmpDir == null) {
        System.err
            .println("Please make sure you have specified \"parserHomeDir, dataPath, outputDir and tmpDir\" in file \"env.jrap\".");
        System.exit(-1);
      }
      ...
      if (!(new File(parserHomeDir).exists()
        ...
        System.err
            .println("Please make sure \"parserHomeDir, dataPath, outputDir and tmpDir\" in file \"env.jrap\" all exist.");
        System.exit(-1);
      }

    } catch (Exception e) {
      System.err
          .println("Something wrong while initialization: can't find the file \"env.jrap\", where environment variables are stored.");
      System.exit(-1);
    }
