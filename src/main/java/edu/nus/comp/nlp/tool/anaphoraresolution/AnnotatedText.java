 ...
 * <p>Description: Class to deal with Charniak parser output.</p>
...
public class AnnotatedText {
  ...
  Vector NPList = new Vector(); //DefaultMutableTreeNode instance inside.overlapping allowed
  Vector SNPList = new Vector();//DefaultMutableTreeNode instance inside.overlapping disallowed
  Vector PRPList = new Vector();//DefaultMutableTreeNode instance inside.overlapping disallowed
  ...
  public void segment() {
    ...
    if (text != null) {
      List sentenceList = Arrays.asList(text.toString().split("\\(S1 "));
      ...
      while (iterator.hasNext()) {
        ...
        if (sentence.trim().length() > 0) {
          ...
          GlobalList.add(Util.analyseTagWordPairs(sentence, new Vector(), sIdx));
  ...
  public Vector getNPList() {
    if (NPList.size() == 0) {
      System.err.println("NPList empty. Ensure that the parser is working. If it is, run segment() first. ");
    }
    return NPList;
  }

  public Vector getPRPList() {
    if (PRPList.size() == 0) {
  ...
  private Vector extractNP() {
    Vector NPVec = new Vector();
    Enumeration enumeration = rootNode.preorderEnumeration();//refer to 'Need preorder here.' line
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (tagWd != null) {
        ...
        if (tagWd.getTag().startsWith("N")
          ...
          //which in turn follows a NP(subject?) who has a single child tagging as 'EX'
          ...
          /**********************************************************/
          //set 'directObj' true if it's the only NP child of a VP, or second NP of a VP
          ...
          DefaultMutableTreeNode h = findFirstChildNode(node, "N", -1,true); //for 'NP'
          if (h == null) {
          ...
          if(node.getChildCount()>1){
            ...
            while(emu.hasMoreElements()){
              DefaultMutableTreeNode hPeople = (DefaultMutableTreeNode)emu.nextElement();
            ...
            while(allNNP && emu.hasMoreElements()){
              DefaultMutableTreeNode hPeople = (DefaultMutableTreeNode)emu.nextElement();
              TagWord hPTw = (TagWord)hPeople.getUserObject();
              //lable the first name, if there is one, as the head of a NP representing a people
              if ( HumanList.isFemale(hPTw.getText()) || HumanList.isMale(hPTw.getText())){
          ...
          if(argH != null){
            ...
            while(((TagWord)argH.getUserObject()).getTag().equalsIgnoreCase("VP")){
          ...
          if (parentTag.startsWith("VP")) {
            ...
            while(((TagWord)upperNode.getUserObject()).getTag().equalsIgnoreCase("VP")
                  ||(((TagWord)upperNode.getUserObject()).getTag().equalsIgnoreCase("S")
          ...
          if (parentTag.startsWith("PP")) {
            ...
            if(grandParentNode.getUserObject()!=null){
              if (((TagWord)grandParentNode.getUserObject()).getTag().startsWith("VP")) {
                argH = findFirstChildNode( (DefaultMutableTreeNode) grandParentNode.
                                          getParent(), "NP", 1);
                if (argH != null) {
          ...
          if( (argadjNode = tagWd.getArgumentHead()) != null){
            ...
            //deepest ancestorNode NP and its containHosts and the deepest VP ancestorNode; Need preorder here.
            ...
            while(pNode!=null){
              ...
              if( (!gotNP) && ((TagWord)pNode.getUserObject()).getTag().startsWith("NP") ){
                tagWd.setContainHost(((TagWord)pNode.getUserObject()).getContainHost());
              }else if( (!gotVP) && ((TagWord)pNode.getUserObject()).getTag().startsWith("VP") ){
          ...
          if (parentTag.startsWith("PP")) {
            DefaultMutableTreeNode previousSiblingNodeOfParent = parentNode.getPreviousSibling();

            if( previousSiblingNodeOfParent != null
                && previousSiblingNodeOfParent.getUserObject() != null){
              if (((TagWord)previousSiblingNodeOfParent.getUserObject()).getTag().startsWith("NP")) {
                if(previousSiblingNodeOfParent.getChildCount()>1){
                  DefaultMutableTreeNode firstCousin = (DefaultMutableTreeNode)previousSiblingNodeOfParent.getChildAt(0);
                  DefaultMutableTreeNode secondCousin = (DefaultMutableTreeNode)previousSiblingNodeOfParent.getChildAt(1);
                  if(((TagWord)firstCousin.getUserObject()).getTag().startsWith("NP")
                     &&((TagWord)secondCousin.getUserObject()).getTag().startsWith("N")){
                    if(!firstCousin.isLeaf()){
                      DefaultMutableTreeNode posNode = findFirstChildNode(firstCousin,"POS",1);
                      if(posNode!=null){
                        tagWd.setNPDomainHost((DefaultMutableTreeNode)posNode.getPreviousSibling());
                      }
                    }
                  }
                }
          ...
          /**********************************************************/
          //connect with third person Determiner (PRP$): the PRP$ sibling of this NP
          Enumeration siblingNodes = parentNode.children();
          while(siblingNodes.hasMoreElements()){
            DefaultMutableTreeNode siblingNode = (DefaultMutableTreeNode)siblingNodes.nextElement();
            if(siblingNode == node){
              //Finish scanning the leading siblings
              break;
            }else if(((TagWord)siblingNode.getUserObject()).getTag().equals("PRP$")){
          ...
          /**********************************************************/
          //If the tag starts with "NN" (NN NNS NNP NNPS), detect whether it has a sibling with similar tag
          if (tagWd.getTag().startsWith("NN")) {
            DefaultMutableTreeNode siblingNode = node.getPreviousSibling();
            if(siblingNode != null){
              if ( ( (TagWord) siblingNode.getUserObject()).getTag().startsWith("NN")){
          ...
          /**********************************************************/
          //First try to set number of the NP, based on the head of the VP related
          DefaultMutableTreeNode siblingVPNode = this.findFirstChildNode(parentNode, "VP", 1);
          if(siblingVPNode != null){
            DefaultMutableTreeNode vpHead = ((TagWord)siblingVPNode.getUserObject()).getHead();
            if(vpHead != null){
              String siblingVPHeadTw = ( (TagWord) vpHead.getUserObject()).getTag();
          ...
          //check the existance of potential ancestor NP (to decide whether it's a head)
          ...
          if (tagWd.getTag().startsWith("PRP")) {
            //for (NP (PRP xxx))
            //copy all the attributes from its parent node while has no siblings
          ...
          //set head
          DefaultMutableTreeNode h = findFirstChildNode(node, "N", -1); //for 'NP'
          ...
          //set head
          DefaultMutableTreeNode h = findFirstChildNode(node, "V", 1); //for 'V'
  ...
  public void buildSNPList() {
    //bug cannot handel single NP
    Iterator iterator = NPList.listIterator();
    if(!iterator.hasNext()){
      return;
    }
  ...
  private void buildParseTree() {
    for (int i = 0; i < sents.size(); i++) {
      ...
      rootNode.add(Util.convertSentenceToTreeNode(i, aTaggedSentence, "(", ")"));
    }
  }
  ...
  protected void test() {
    //Usage:
    //AnnotatedText aText = new AnnotatedText(Util.read("$HOME/input.3.par").toString());
    //aText.test();
    ...
    while (enumeration.hasMoreElements()) {
      ...
      System.out.println(tagWd.getContent());
    }
  }

  public void test(Vector vec) {
   ...
   */
  public static DefaultMutableTreeNode findFirstChildNode(DefaultMutableTreeNode parentNode,String taghead, int direction) {
 ...
 * @param taghead
 * @param direction: Binary: +1 (from right) or -1 (from left)
 * @param recursive: true to do the search recursively until a leaf node matches the requirement is found
 * @return the first child from left(+1)/right(-1) that satisfies taghead
 */
public static DefaultMutableTreeNode findFirstChildNode(DefaultMutableTreeNode parentNode,String taghead, int direction, boolean recursive) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    DefaultMutableTreeNode checkee = (DefaultMutableTreeNode) parentNode.getChildAt(
        idx);
  ...
  public static void main(String[] args) {
    AnnotatedText annotatedText1 = new AnnotatedText("");
  }
}
