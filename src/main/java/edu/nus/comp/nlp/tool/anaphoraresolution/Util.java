...
public class Util {
  ...
  /**
   * Filter out none-sentence part in TREC plain text corpus
   * 27	nusmml2003	XIE19960317.0175	Like most of the world 's subway systems , the lines operate at a loss .
   * @param fileName
   * @return
   */
  ...
  public static int findMatcher(String target, String matcheeL, String matcherR) {
    int loc = 0;
    int depth = 0;
  ...
  /**
   * Convert the output of Charniak parser for ~A single sentence~ into a TreeNode.
   * */
...
//        System.err.println("cmd /c  "+command + " " + System.getProperty("parserOption") + dataDir + " " + inputFile);
  ...
  public static Vector resolverV1(Vector aNPList, Vector aPRPList) {
          ...
          if (matchLexcialAnaphor(npTw, prpTw)) {
            ...
            if (prpNode.getSiblingCount() == 1) {
              ...
              DefaultMutableTreeNode parentNode
                  = (DefaultMutableTreeNode) prpNode.getParent();
              if (parentNode != null) {
            ...
            if (System.getProperty("referenceChain").equals("false")) { //true/undefine by default
              ...
              resultsOut.add(new CorreferencialPair(npTw.getAntecedent(), prpTw));
            }
    ...
    if (results.toString().length() > 2) {
      toWrite = results.toString().substring(2, results.toString().length() - 1);
    }
    else {
      toWrite = "null";
    }
    ...
    if (System.getProperty("write resolving results") == null ||
        System.getProperty("write resolving results").equals("true")) { //make use of shortcut
      String resultsFileName = System.getProperty("outputDir") + File.separator +
    ...
    Util.errLog("***********Head of Results**************\n" + toWrite +
                "\n***********End of Results***************\n");
    Util.showMessage("********Anaphor-antecedent pairs*****\n" + toWrite + "\n",
  ...
  private static Object getBestCandidate(Object[] sortedCandidates, TagWord tw) {
    ...
    if (sortedCandidates.length == 0) {
      ...
      if (tw0.getTmpSalience() > tw1.getTmpSalience()) {
        ...
        /* if(Math.abs(tw0.getDepth() - tw.getDepth()) < Math.abs(tw1.getDepth() - tw.getDepth()) ){
           //closer depth
           obj = tw0;
  ...
  private static boolean matchLexcialAnaphor(TagWord npTw, TagWord lexTw) {
    ...
    boolean judge = false;
    DefaultMutableTreeNode npNode = npTw.getNPRepresentation().getNodeRepresent();
  ...
  /**
   * A morphological filter for ruling out anaphoric dependence of a pronoun on an NP due
   * to non-agreement of person, number, or gender features.
  ...
  static String processResult(Object np, Object referer) {
    ...
    if (np == null) {
      refereeStr = "NULL";
      /*}else if( ((TagWord)np).getAntecedent().getNPRepresentation().getNodeRepresent().isNodeChild(
           ((TagWord)referer).getNPRepresentation().getNodeRepresent())   ){
        //self refering
        refereeStr = "NULL";
        //Todo: Util.java processResult()
        //April 29, 2004: In
        //"But they have 200 % of the (high-end) market they want, which is not bad"
        //One "they" is regarded as another "they"'s child, which is to be looked at.
  ...
  static public String substitutionV0(DefaultMutableTreeNode root,
    ...
    while (iterator.hasNext()) {
      ...
      //Locate the beginning position of the referer in the possiblly altered sentence.
  ...
  static public String substitution(DefaultMutableTreeNode root, Vector result,
    ...
    while (sentences.hasMoreElements()) {
      ...
      if (partialSubstitution) {
        String spli = "[^a-zA-Z]+";
        if ( (sentID == -1) && arrayMatch(aSentStr.split(spli), sent.split(spli))) {
          sentID = i;
        }
      }
